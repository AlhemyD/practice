import sys, requests, json, os, glob, threading
...
#        logger.info(f"Parsed data for station {pub.station} successfully. Starting scheduler")
        ...
        try:
            ...
            return {"error":f"Cought an Exception for station {pub.station} - Exception - {e}"}
...
#            logger.warning(f"Ended publishing on {pub.station} due to {KeyboardInterrupt}")            
        ...
        pub.disconnect()
        pub.loop_stop()
        return {"error":f"Unknown error during request for station {pub.station}"}
publi_thread=threading.Thread(target=publi, args=(date,pattern,data_path))
publi_thread.start()
publi_thread.join()
