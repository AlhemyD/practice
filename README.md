...
## Архитектура проекта ##
![Архитектура](project_scheme.png)
...
# Если ваш клиент расположен на другой ВМ, но в той же сети:

2.14 Откройте файл ``/etc/mosquitto/mosquitto.conf`` и в конец файла пишете следующие строки:
...
2.15 Откройте файл ``sub.py`` и вместо localhost в строке номер 3 вставьте ip адрес сервера

``broker="localhost"`` -> ``broker="192.168.0.103"``
# Если ваш клиент находится на той же ВМ, что и сервер, просто переходите к шагу 2.2
...
Данный скрипт автоматизирует все процессы от скачивания до публикации
...
После завершения загрузки архив распакуется с помощью вызванного bash скрипта, если же процесс распаковки завершится с ошибкой, то выведется сообщение об ошибке. После успешной распаковки файлы форматируются из формата .crx в формат .rnx с помощью функции reformat_crx_to_rnx. после того, как формат данных будет изменен и файл готов для парсинга, запускается скрипт для создания демона для scriprnx.py, с помощью которого будет остуществлен парсинг. После запуска данного демона создаются сервисы для pub.py, которые будут поддерживать отправку данных от файлов.
...
Сценарий работы скрипта pub.py включает в себя следующие шаги: получение аргументов командной строки (включая имя файла и дату), формирование шаблона имени файла и определение пути к данным на основе переданной даты. Также скрипт определяет функцию publi для публикации данных, инициализирует планировщик BackgroundScheduler, получает имя файла данных согласно шаблону, инициализирует объект Publisher для публикации данных. Далее устанавливается соединение и запускается цикл публикации данных. Скрипт также отправляет запрос на парсинг данных по указанной ссылке, настраивает планировщик для автоматической публикации данных в определенное время в случае получения ответа с данными для парсинга. После запуска планировщика скрипт ожидает выполнения задач и в случае возникновения исключений или прерывания работы, завершает соединение и останавливает публикацию данных.

## Subscriber ##

Для получения данных необходимо запустить sub.py. Этот скрипт является клиентом MQTT, который подключается к брокеру на локальном сервере (localhost) и подписывается на тему "station/data". При получении сообщения на этой теме, скрипт вызывает функцию on_message, которая парсит данные из полученного сообщения и выводит их в консоль. После подключения к брокеру и установки подписки на тему "station/data", скрипт начинает бесконечный цикл, в течение которого он ожидает новые сообщения в течение 1800 секунд (половины часа). Если пользователь прерывает выполнение скрипта (например, нажимает Ctrl+C), скрипт завершает подписку и закрывает соединение с брокером. Таким образом, работа этого скрипта заключается в постоянном прослушивании темы "station/data", обработке полученных сообщений и выводе их содержимого в консоль.
...
Логи записываются как в консоль, так и в файл my_app.log. Уровень логирования установлен на INFO.
